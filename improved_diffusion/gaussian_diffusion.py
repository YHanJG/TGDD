...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        print(f"num_timesteps.shape:{betas.shape[0]}")
    ...
    def training_losses(self, model, *args, **kwargs):
        if self.training_mode == 'e2e':
            is_join = False
            return self.training_losses_e2e(model,is_join, *args, **kwargs)
        elif self.training_mode == 'e2e-join':
            is_join = True
            return self.training_losses_e2e(model,is_join, *args, **kwargs)
        elif self.training_mode == 'e2e-two-way':
            return self.training_losses_Double_e2e(model, *args, **kwargs)
        elif self.training_mode == 'e2e-two-way-independent':
            return self.training_losses_Double_independent_e2e(model, *args, **kwargs)
    ...
    def p_mean_variance(
        ...
        if self.is_two_way:
            ...
            model_output , a= model(x, self._scale_timesteps(t), desc[0], desc[1], **model_kwargs)
            print(model_output.shape)
            print("yes")
        else:
            a , model_output = model(x, self._scale_timesteps(t), desc[0], desc[1], **model_kwargs)
...
# used
    def training_losses_e2e(self, model,is_join, micro, t, model_kwargs=None, noise=None,):
        """
        ...
        if is_join:
            add_x_start_mean = model.model.module.get_add_embeds(add_ids)
            print(add_x_start_mean.shape)
            std_add = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
                                   th.tensor([0]).to(add_x_start_mean.device),
                                   add_x_start_mean.shape)
        # th.tensor([0]).to('cuda')
        ...
        if is_join:
            add_x_start = self.get_x_start(add_x_start_mean,std_add)
        ...
        join_get_logits = model.model.module.get_logits_join
        terms = {}
        ...
        if self.loss_type == LossType.E2E_KL:
            ...
            if not is_join:
                ...
                target = {
                    ...
                    ModelMeanType.START_X: x_start,
                    ModelMeanType.EPSILON: noise
                }[self.model_mean_type]   #this is exactly x_start
                # print(model_output.shape ,target.shape , x_start.shape)

                assert model_output.shape == target.shape == x_start.shape
                terms["mse"] = mean_flat((target - model_output) ** 2)
                # print( terms["mse"])
                model_out_x_start = self.x0_helper(model_output, x_t, t)['pred_xstart'] #this is exactly model_output
                t0_mask = (t == 0)
                ...
                decoder_nll = self.token_discrete_loss(x_start, get_logits,
                        input_ids)
                ...
                add_output,model_output = model(x_t, self._scale_timesteps(t),desc_state,desc_mask)
                loss_add = mean_flat((add_output - add_x_start) ** 2)
                
                decoder_nll_add = self.token_discrete_loss(add_x_start, join_get_logits,
                        add_ids)
                ...
                terms["loss"] = terms["mse"] + (decoder_nll + tT_loss) + loss_add + decoder_nll_add
                return terms,loss_add 
        else:
            raise NotImplementedError(self.loss_type)

        return terms
    def training_losses_Double_independent_e2e(self, model, micro, t, model_kwargs=None, noise=None,):
        """
        Compute training losses for a single timestep.
        ...
        # print(input_ids[1,:].unsqueeze(0))
        # print(add_ids[1,:].unsqueeze(0))
        # print(combined_ids[1,:].unsqueeze(0))
        # assert 1==2
        assert(corrupt_ids.shape==input_ids.shape)
        if t.max()>self.maxt:
            self.maxt = t.max()
            print('Recieving max t:{}'.format(self.maxt))
        ##########################################
        smile_x_start_mean = model.model.module.get_embeds(input_ids)
        self_x_start_mean = model.model.module.get_add_embeds(add_ids)
        std_self = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
                                   th.tensor([0]).to(self_x_start_mean.device),
                                   self_x_start_mean.shape)
        std_smile = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
                                   th.tensor([0]).to(smile_x_start_mean.device),
                                   smile_x_start_mean.shape)
        
        # print(std.shape, )
        # x_start_log_var = 2 * th.log(std)
        smile_x_start = self.get_x_start(smile_x_start_mean, std_smile)
        self_x_start = self.get_x_start(self_x_start_mean, std_self)
        # batchsize*256*32
        smile_x_start.shape
        # batchsize*256*32
        self_x_start.shape
        # print(x_start_mean.shape, x_start.shape)
        x_start = th.cat((smile_x_start, self_x_start), dim=1)
        if noise is None:
            noise = th.randn_like(x_start)
        x_t = self.q_sample(x_start, t, noise=noise) # reparametrization trick.
        
        smile_get_logits = model.model.module.get_logits
        self_get_logits = model.model.module.get_add_logits
        terms = {}
        model_output = model(x_t, self._scale_timesteps(t),desc_state,desc_mask)
        model_output.shape
        out_smiles = model_output[:, :256 ,:]
        out_selfies = model_output[:, 256: ,:]
        if self.model_var_type in [
            ModelVarType.LEARNED,
            ModelVarType.LEARNED_RANGE,
        ]:
        ...
        assert model_output.shape == target.shape == x_start.shape
        terms["mse"] = mean_flat((smile_x_start - out_smiles) ** 2) + mean_flat((self_x_start - out_selfies) ** 2)
        # print( terms["mse"])
        model_out_x_start = self.x0_helper(model_output, x_t, t)['pred_xstart'] #this is exactly model_output
        out_smiles_x_start = model_out_x_start[:, :256,:]
        out_selfies_x_start = model_out_x_start[:, 256:,:]
        t0_mask = (t == 0)
        t0_loss = mean_flat((smile_x_start_mean - out_smiles_x_start) ** 2) + mean_flat((self_x_start_mean - out_selfies_x_start) ** 2)
        # print(terms["mse"].shape, )
        terms["mse"] = th.where(t0_mask, t0_loss, terms["mse"])
        ...
        decoder_nll = self.token_discrete_loss(out_smiles, smile_get_logits,
                input_ids) + self.token_discrete_loss(out_selfies, self_get_logits,
                add_ids)
        ...
        if "vb" in terms:
            ...
            terms["loss"] = terms["mse"] + (decoder_nll + tT_loss)
        return terms
    def training_losses_Double_e2e(self, model, micro, t, model_kwargs=None, noise=None,):
        """
        Compute training losses for a single timestep.
        ...
        print(add_ids.shape)
        combined_ids = th.cat((input_ids, add_ids), dim=1)
        # [64, 512]
        # print(combined_ids.shape)
        # print(input_ids[1,:].unsqueeze(0))
        # print(add_ids[1,:].unsqueeze(0))
        ...
        if t.max()>self.maxt:
            ...
            print('Recieving max t:{}'.format(self.maxt))
        ##########################################
        x_start_mean = model.model.module.get_embeds(combined_ids)
